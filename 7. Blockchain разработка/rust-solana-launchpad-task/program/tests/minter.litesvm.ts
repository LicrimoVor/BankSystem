...
function assertFailure(res: any) {
	if (typeof res?.err === "function") {
		const err = res.err();
		expect(err, `tx unexpectedly succeeded: ${res.toString()}`).to.not.be
			.null;
		return;
	}
...
function buildTx(
	...
	tx.sign(
		...[
			feePayer,
			...ixs.flatMap((ix) => (ix as any)._additionalSigners ?? []),
		],
	);
	return tx;
}
