...
fn fix(bytes_to_fix: &str, set_of_labels: &HashSet<String>, id: usize) -> String {
    ...
    bytes_to_fix
        ...
        .map(|orig_word| {
            ...
            shall_start_from_capital = *orig_word.as_bytes().last().unwrap() == ('.' as u8);
            if let Some(label) = set_of_labels
                .iter()
                .find(|label| to_lower_iter(&label).eq(to_lower_iter(orig_word)))
            {
...
fn run_parallel(string: String, set_of_labels: &HashSet<String>) -> String {
    std::thread::scope(|scope| {
        ...
        let threads = (0..threads_count).map(|i| {
            let part = parts[i];
            scope.spawn(move || fix(part, set_of_labels, i + 1))
        });

        for thread in threads {
            result.push_str(&thread.join().unwrap());
        }
...
fn main() {
    ...
    result = run_parallel(mix.to_string(), &dict);
